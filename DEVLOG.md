...
| v0.4.2 | Two fixes. (1) **Removed the VAD noise gate** — Silero VAD (v0.4.1) was clipping real speech to silence on some mics, so dictation came back empty / as the "Thank you." silence-hallucination. `build-app.sh` no longer bundles the VAD model; the `enable_vad` code stays but is model-gated (dormant). Re-add with a tuned, lower threshold after real testing. (2) **Fixed crash on quit** — whisper.cpp's Metal teardown (`ggml_metal_device_free → ggml_metal_rsets_free`) aborts at `exit()` when its residency-set init is still racing; on Quit we now `libc::_exit(0)` to skip the C++/atexit teardown (OS reclaims everything). A regression from the 0.16 Metal upgrade, caught via a user crash report. | User couldn't dictate (silence) and hit a crash dialog on every quit. Reliability of dictation outranks noise suppression for a clinical tool. |
| v0.4.1 | Two changes. (1) **English-only lock** — `set_detect_language(false)` next to the existing `set_language("en")`, plus `is_cjk()` stripping of Chinese/Japanese/Korean Unicode blocks from `scrub()` output (narrow: leaves Greek/accented-Latin/punctuation intact). (2) **Noise gate (VAD)** — bundles the ~860 KB Silero VAD model and enables whisper-rs 0.16 integrated VAD (`set_vad_model_path` + `enable_vad`), so only detected speech is transcribed; optional (runs without it if the model is absent). | User saw Chinese/Japanese characters appear (Whisper language drift on noise/accent) and gibberish from background noise while traveling. Pinning English kills the drift; the VAD gate stops noise/silence from being hallucinated into words. |
